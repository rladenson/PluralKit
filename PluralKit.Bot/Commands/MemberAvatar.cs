                                  ...
                                  MemberGuildSettings? guildData)
    {
        ...
        if (string.IsNullOrEmpty(currentValue) || !canAccess)
        {
            ...
            if (location == MemberAvatarLocation.MemberWebhook)
                throw new PKError(
                    $"This member does not have a proxy avatar set. Type `pk;member {await target.Reference(ctx)} avatar` to see their global avatar.");

            if (location == MemberAvatarLocation.Server)
                throw new PKError(
                    $"This member does not have a server avatar set. Type `pk;member {await target.Reference(ctx)} avatar` to see their global avatar.");
        }
        ...
        if (target.System == ctx.System?.Id)
            eb.Description($"To clear, use `pk;member {await target.Reference(ctx)} {location.Command()} clear`.");
