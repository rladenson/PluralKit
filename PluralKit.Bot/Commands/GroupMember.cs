...
public class GroupMember
{
    ...
    public async Task ListMemberGroups(Context ctx, PKMember target)
    {
        ...
        {
            msg +=
                $"\n\nTo add this member to one or more groups, use `pk;m {reference} group add <group> [group 2] [group 3...]`";
            if (groups.Count > 0)
                msg +=
                    $"\nTo remove this member from one or more groups, use `pk;m {reference} group remove <group> [group 2] [group 3...]`";
        }
    ...
    public async Task ListGroupMembers(Context ctx, PKGroup target)
    {
        ...
        await ctx.CheckSystemPrivacy(targetSystem.Id, target.ListPrivacy);
        ...
        await ctx.RenderMemberList(await ctx.LookupContextFor(target.System), target.System, title.ToString(),
            target.Color, opts);
    }
