...
public class GroupMember
{
    ...
    public async Task ListMemberGroups(Context ctx, PKMember target)
    {
        ...
        {
            msg +=
                $"\n\nTo add this member to one or more groups, use `pk;m {await target.Reference(ctx)} group add <group> [group 2] [group 3...]`";
            if (groups.Count > 0)
                msg +=
                    $"\nTo remove this member from one or more groups, use `pk;m {await target.Reference(ctx)} group remove <group> [group 2] [group 3...]`";
        }
